...
const form = ref({
  item_id: null,
  room_number: '', name: '', category: '', category_type: 'room',
  max_guests: 2, price: null, quantity: 1,
  description: '', status: 'Available',
  date: '', time: '', promo: false, images: []
})
...
const handleSave = () => {
  ...
  emit('save', { ...form.value, category: form.value.category || form.value.name || '', images: [...imagePreviews.value], primaryImageIndex: primaryImageIndex.value })
  close()
}
...
const processFiles = (files) => {
  ...
  Array.from(files).forEach(file => {
    if (!validTypes.includes(file.type)) { alert(`${file.name} is not a supported image type.`); return }
    if (file.size > 5 * 1024 * 1024)    { alert(`${file.name} is too large (max 5MB)`); return }
    const reader = new FileReader()
    reader.onload = (ev) => {
      imagePreviews.value.push(ev.target.result)
      if (imagePreviews.value.length === 1) primaryImageIndex.value = 0
    }
