...
// Counter methods
const incrementAdults = () => guest.value.adults++
const decrementAdults = () => { if (guest.value.adults > 1) guest.value.adults-- }
const incrementChildren = () => guest.value.children++
const decrementChildren = () => { if (guest.value.children > 0) guest.value.children-- }
...
const payNow = async () => {
  // Validation
  if (!guest.value.firstName || !guest.value.lastName || !guest.value.phone || 
      !guest.value.email || !guest.value.address || !guest.value.city || !guest.value.postal) {
  ...
  const finalBooking = {
    ...
    guest: {
      ...
      fullName: `${guest.value.firstName} ${guest.value.lastName}`,
      adults: guest.value.adults,
      children: guest.value.children,
      arrivalTime: guest.value.arrivalTime,
      specialRequests: guest.value.specialRequests || '',
      phone: '+63' + guest.value.phone,
      email: guest.value.email,
      address: `${guest.value.address}, ${guest.value.city} ${guest.value.postal}`
    },
...
onMounted(() => {
  ...
  const { items = [], checkIn, checkOut, selectedPayment: savedPayment = 'gcash' } = data
  ...
  bookedItems.value = processedItems
